...
class GraphApiClient : AzureDevOpsApiClient {
    ...
    # Get a list of all users in a given scope.Since the list of users may be large, results are returned in pages of users.  If there are more results than can be returned in a single page, the result set will containt a continuation token for retrieval of the next set of results.
    ...
    [PSObject] GetUser([bool] $useTargetProject, [string] $userDescriptor) {
     ...
     # Disables a user.Calls to GET .../users/{userDescriptor} will return the user with disabled marked true. Other GET calls, which may return multiple users, can control whether or not deleted users should be returned using the disabled flag, which defaults to false.
